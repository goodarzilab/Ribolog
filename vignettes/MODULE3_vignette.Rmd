...
rr_LMCN <- readRDS("../data-raw/rr_LMCN")
library(knitr)
require(Ribolog)
require(ggrepel)
require(xlsx)
qc.tests <- read.table("../data-raw/QC_tests.txt", sep = ",", header=TRUE)
names(qc.tests) <- c("QC Test",	"Multiple replicates per sample",	"Before or After TER significance test")
kable(qc.tests)
```
...
saveRDS(rr_LMCN.v2, "../data-raw/rr_LMCN.v2")
```
...
```{r cache = TRUE}
rr_LMCN.v2.pairwise <- Ribolog::TER_all_pairs(x = rr_LMCN.v2.split, design = sample_attributes_LMCN, outcome = "read_type", uniqueID = "replicate_name", groupID = "cell_line")
```
